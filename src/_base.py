...
from typing import Any, ClassVar, Generic, TypedDict, TypeVar, cast, final, overload
...
class SingletonMeta(Generic[_Self], ABCMeta, type):
    ...
    @staticmethod
    def __call__(__cls, *args, **kwds) -> _Self:
        """This has a role of `__new__` in `cls`"""
        if __cls._instance is None:
            return __cls._create_and_register_new_instance(*args, **kwds)

        return __cls._instance
...
class Singleton(SimpleSingleton, ABC, metaclass=InvalidableSingletonMeta):
    """
    Does not work with dataclass - at least dataclass' __init__ is discarded
    """
